...
{
    public partial class UC_Item4 : UserControl
    {
        ...
        public UC_Item4(string fName, decimal Price)
        {
            InitializeComponent();
            txtName.Text = fName;
        ...
        public UC_Item4 Item()
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql = "SELECT fName, Price FROM ITEM WHERE ID = 4";
                ...
                using (SqlCommand command = new SqlCommand(sql, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            UC_Item4 itemControl = new UC_Item4(fName, Price);
