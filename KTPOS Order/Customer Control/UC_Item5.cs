...
{
    public partial class UC_Item5 : UserControl
    {
        ...
        public UC_Item5(string fName, decimal Price)
        {
            InitializeComponent();
            txtName.Text = fName;
        ...
        public UC_Item5 Item()
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string sql = "SELECT fName, Price FROM ITEM WHERE ID = 5";
                ...
                using (SqlCommand command = new SqlCommand(sql, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            UC_Item5 itemControl = new UC_Item5(fName, Price);
